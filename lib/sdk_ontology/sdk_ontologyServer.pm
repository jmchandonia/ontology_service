package sdk_ontology::sdk_ontologyServer;
...
sub call_method {
    ...
    my $ctx = sdk_ontology::sdk_ontologyServerContext->new($self->{loggers}->{userlog},
...
{
    # Service sdk_ontology requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for sdk_ontology but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = sdk_ontology::sdk_ontologyServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service sdk_ontology.");
    }
...
package sdk_ontology::sdk_ontologyServerContext;

use strict;

=head1 NAME

sdk_ontology::sdk_ontologyServerContext
...
package sdk_ontology::sdk_ontologyServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = sdk_ontology::sdk_ontologyServer->new(
